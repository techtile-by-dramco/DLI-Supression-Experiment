...
1. A general system setup.
   1. Make sure the rpi OS and software is up-to-date.
   2. Pull the experiment repo and check that all experiment-specific requirements are met.

2. Run the experiment (can be an iterative process)
   1. Pull the repo (if changes to certain run settings have been made).
   2. Run the experiment scripts on the rpi's (output can be registered asynchronously)
