...
MIN_PLL_VAR: Optional[float] = None
CAPTURE_TIME: Optional[float] = None
TX_TIME: Optional[float] = None
SERVER_IP: Optional[str] = None

try:
    cal_path = os.path.join(os.path.dirname(__file__), "cal-settings.yml")
...
def rx_ref(usrp, rx_streamer, quit_event, duration, result_queue, start_time=None):
    ...
    logger.debug(
        "GAIN IS CH0: %s CH1: %s", usrp.get_rx_gain(0), usrp.get_rx_gain(1)
    )
...
def tx_phase_coh(usrp, tx_streamer, quit_event, phase_corr, at_time, long_time=True):
    ...
    logger.debug("amplitudes: %s", amplitudes)
    logger.debug("TX Gain: %s", FREE_TX_GAIN)
...
def main():
    ...
    try:
        ...
        # FPGA file path
        fpga_path = os.path.join(script_dir, "usrp_b210_fpga_loopback.bin") #TODO check to put it in the config instead of here.
        ...
        # -------------------------------------------------------------------------
        # STEP 0: Preparations / Setup URSP
        # -------------------------------------------------------------------------

        # Set up TX and RX streamers and establish connection
        tx_streamer, rx_streamer = setup(usrp, connect=True)
        ...
        # -------------------------------------------------------------------------
        # STEP 2: Load cable phase correction from YAML configuration (if available)
        # -------------------------------------------------------------------------
        phi_cable = 0
        with open(
            os.path.join(os.path.dirname(__file__), "ref-RF-cable.yml"),
            "r",
            encoding="utf-8",
        ) as phases_yaml:
            try:
                ...
                if HOSTNAME in phases_dict.keys():
                    ...
                    logger.debug("Applying phase correction: %s", phi_cable)
                else:
                    logger.error("Phase offset not found in ref-RF-cable.yml")
            except yaml.YAMLError as exc:
                logger.error("Error parsing 'ref-RF-cable.yml': %s", exc)
                sys.exit(ExitCode.YAML_PARSING_ERROR)

        # -------------------------------------------------------------------------
        # STEP 3: Extract transmit weights from YAML configuration
        # -------------------------------------------------------------------------
        ...
        with open(
            ...
            try:
                ...
                if HOSTNAME in weights_dict.keys():
                    for c_weights in weights_dict[HOSTNAME]:
